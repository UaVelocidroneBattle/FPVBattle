...
public class TrackService
{
    ...
    private List<ParsedTrackModel> GetCandidateTracks(IEnumerable<ParsedMapModel> maps)
    {
        ...
        var filteredTracks = allTracks.Where(t => trackFilter.IsTrackGood(t)).ToList();

        _log.Debug("Filtered to {FilteredCount} tracks (from {TotalCount} total)",
    ...
    private async Task<List<string>> GetUsedTrackIdsAsync()
    {
        ...
        _usedTrackIds = ids;
        _log.Debug("Found {UsedTrackCount} tracks used in the last 3 months", ids.Count);
